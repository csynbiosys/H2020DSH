...
function [resPI] = PIdent4SBL(inputs)

inputsPre = inputs;
clear inputs;
inputs={};
results_folder = strcat('PracticalIdentifiabilityAnalysis',datestr(now,'yyyy-mm-dd-HHMMSS'));
short_name     = strcat('PIdent',inputsPre.model.matlabmodel_file);
...
inputs.rid.conf_ntrials=500; % Reduce?     

[inputs,privstruct] = AMIGO_Prep(inputs);
inputsPI=inputs;
resPI = AMIGO_RIdent(inputs);

save(['PIdent_',inputsPre.model.matlabmodel_file,'_',date,'.mat'], 'resPI', 'inputsPI')
