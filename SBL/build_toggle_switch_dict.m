function [Phi,Phi_val] = build_toggle_switch_dict(x)
%dictionary mtx
...
%% state 1
kinetics.formula = @(x,p) 1./(1+((x(:,2)./p.p_tetr).*(1./1+(x(:,4)./p.p_atc).^p.n_atc)).^p.n_tetr);
...
[Phi_val_1,Phi_1] = dict_generator(x,kinetics);


kinetics.formula = @(x,p) 1./(1+((x(:,1)./p.p_tetr).*(1./1+(x(:,3)./p.p_atc).^p.n_atc)).^p.n_tetr);
kinetics.name = 'hill(hill)';
kinetics.p.p_tetr = 1:1:20;
kinetics.p.p_atc = 0.1:1:10;
kinetics.p.n_atc = 1:2;
...
[Phi_val_2,Phi_2] = dict_generator(x,kinetics);
