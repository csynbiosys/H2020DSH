...
kinetics_state1(4).p.p_tetr = 0.1:1:10;
...
kinetics_state2(4).formula = @(x,u,p) 1./(1+((x(:,1)./p.p_tetr).*1./(1+(u(:,1)./p.p_atc).^p.n_atc)).^p.n_tetr);
kinetics_state2(4).name = 'hill(hill)';
kinetics_state2(4).p.p_tetr = 0.01:1:10;
kinetics_state2(4).p.p_atc = 0.01:1:10;
