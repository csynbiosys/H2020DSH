...
for iFilesSelected=1:length(modelFiles_selected)
    ...
    %% transform fields
    models_inputs(iFilesSelected).model.input_model_type='charmodelC';
    models_inputs(iFilesSelected).model.st_names = char(models_inputs(iFilesSelected).model.st_names');     % name of states
    models_inputs(iFilesSelected).model.n_st = length(models_inputs(iFilesSelected).model.st_names);        % number of states
    models_inputs(iFilesSelected).model.par_names = char(models_inputs(iFilesSelected).model.par_names');   % name of parameters
    models_inputs(iFilesSelected).model.n_par = length(models_inputs(iFilesSelected).model.par_names);      % number of parameters    
    models_inputs(iFilesSelected).model.par = cell2mat(models_inputs(iFilesSelected).model.par);            % parameters value (row)
    models_inputs(iFilesSelected).model.eqns = char(models_inputs(iFilesSelected).model.eqns);              % model equations
    models_inputs(iFilesSelected).model.stimulus_names = char(models_inputs.model.stimulus_names');         % name of stimulus
    models_inputs(iFilesSelected).model.n_stimulus = length(models_inputs(iFilesSelected).model.stimulus_names); 
    ...
    models_inputs(iFilesSelected).model.name = char(SBL_model_name(1));
    
    data_inputs.exps= load_data(strcat('./Data/NetworkData/',network_name,'.csv'),union(models_inputs(iFilesSelected).exp_training_idx,models_inputs(iFilesSelected).exp_test_idx),column_config);
    ...
    models_inputs(iFilesSelected) = rmfield(models_inputs(iFilesSelected),'exp_info');
